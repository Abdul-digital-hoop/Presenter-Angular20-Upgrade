...
export class TruthOrLieComponent implements OnInit, OnDestroy {
  ...
  @Input('presentationMode') public presentationMode: boolean;
 ...
 truthorliepresenterEnterClick = false; 
  ...
  ngOnInit(): void {
    var truthorLieData = this._workspaceservice.convertDataFormat(this.slideDetails?.slideContentData, 'Options');
    this.truthorLieData = truthorLieData;
    ...
    this.truthorliepresenterEnterClick = this._workspaceservice.changeCorrectAnswerFormat(this.slideDetails?.slideContentData);
    this.shuffleColors();
    this.existingData = this.truthorLieData;
    this.originalData = [...this.truthorLieData];
  ...
  assignOptions() {
    // d3.selectAll(`div#p?ie-${this.slideDetails?.slideId}-${this.viewfrom} svg > *`).remove();
    this.truthorLieData.forEach((value, index) => {
      this.data = [];
      if ((value.truthCount == 0 && value.falseCount == 0) || !this.slideDetails.settings.showInResults) {
        ...
        if (this.truthorliepresenterEnterClick || !this.presentationMode) {
  ...
  private createSvg(index: any, Count: any): void {
    this.svg = d3
      ...
      .append("g")
      .attr("class", "pie-group-" + index+'-'+this.viewfrom+'-'+this.randomId)
      .attr("width", "100%")
      .attr("height", "100%")
      .attr(
        "transform",
  ...
  private updatePaths(data_ready: any, arc: any, index: any): void {
    ...
    paths.enter()
      ...
      .attr("class", (d: { data: { currentpath: string, id: string } }) => `${d.data.currentpath}-${d.data.id}-${this.viewfrom}-${this.randomId}`)
      .transition()
      .duration(1000)
      .attrTween("d", function(d: { startAngle: number, endAngle: number }) {
        const previous = (this as any).__previous || d;
...
private updateLabels(data_ready: any, outerArc: any, index: any): void {
  ...
  labels
    ...
    .attr("class", (d: any) => `${d.data.currentpath}-${d.data.id}-${this.viewfrom}-${this.randomId}`)
...
private defaultChart(): void {
  if ( !this.presentationMode || this.truthorliepresenterEnterClick) {
...
private getCommonFontSizeFromTruthorLieData(): string {
  ...
  updateChart(value: any) {
    ...
    this.truthorLieData.forEach((option, index) => {
      ...
      if ((option.truthCount === 0 && option.falseCount === 0) || !this.slideDetails.settings.showInResults) {
        this.data.push({ 
          id: option.OptionId, 
...
public getDynamicColor(index: number, currentpath: string): string {
  ...
  updateChartWithRandomData(): void {
    try {
      if (this.isFirstUpdate) {
        ...
        this.truthorLieData = this.truthorLieData.map(option => ({
        ...
        const randomIndex = Math.floor(Math.random() * this.truthorLieData.length);
        const isTruth = Math.random() < 0.5; // Randomly choose between truth and false

        this.truthorLieData = this.truthorLieData.map((option, index) => {
          if (index === randomIndex) {
            return {
              ...option,
              truthCount: isTruth ? option.truthCount + 1 : option.truthCount,
              falseCount: !isTruth ? option.falseCount + 1 : option.falseCount
            };
          }
          return option;
        });
      ...
      console.error('Error updating truth or lie data:', error);
  ...
  stopRandomDataUpdates(): void {
    ...
    this.isFirstUpdate = true;
    this.truthorLieData = [...this.originalData];
    this.assignOptions();
    this.cdr.detectChanges();
  }

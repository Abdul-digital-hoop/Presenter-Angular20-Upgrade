import { AfterViewInit, Component, ElementRef, EventEmitter, HostListener, Input, OnInit, Output, Renderer2, ViewChild, OnDestroy, ChangeDetectorRef } from '@angular/core';
...
export class OpenEndedComponent implements OnInit, OnDestroy {
  ...
  constructor(private el: ElementRef, private renderer: Renderer2, private _commanservice:CommanService, public _workspaceservice: WorkspaceService, private cdr: ChangeDetectorRef) {    
    this.screenOptions = "presentationScreen";
  }

  ngOnInit(): void {
    this.screenOptions = "presentationScreen";
    var openEndedData = this._workspaceservice.convertDataFormat(this.slideDetails?.slideContentData,'Options');
    ...
    this.chunkArray(this.defaultData, 3);
  }
  ...
  incrementalScroll(step: number, pause: number) {
    let currentScrollPosition = $('.row1').scrollTop();
    const documentHeight = $(document).height();
  
    const scrollStep = () => {
      if (currentScrollPosition + step < documentHeight) {
  ...
  showOptionDetails(index: number) {
   // document.getElementById(`overflow-scroll-${this.slideDetails?.slideId}-${this.viewfrom}`).classList.remove("overflow");
  ...
  displayAllAnswers() {
    document.getElementById(`overflow-scroll-${this.slideDetails?.slideId}-${this.viewfrom}`).classList.add("overflow");
  ...
  updateChart(value: any) {
    ...
    if(this.isPreviewMode){
      ...
      this.openEndedData = openEndedData.filter(item => item.Answer && typeof item.Answer === 'string' && item.Answer.trim() !== '' && item.ModerateAnswer == false);
    }
    this.openEndedData = this._workspaceservice.profanityWordsChecksForWord(this.openEndedData);
  }
  ...
  chunkArray(arr: any[], size: number) {
    this.defaultDataColumn = Array.from({ length: Math.ceil(arr.length / size) }, (_, index) =>

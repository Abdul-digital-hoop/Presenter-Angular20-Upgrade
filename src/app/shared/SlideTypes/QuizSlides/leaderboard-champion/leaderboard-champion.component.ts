...
export class LeaderboardChampionComponent implements OnInit, AfterViewInit {
  ...
  leaderBoardUsers() {
    ...
    var presentationLevelPlayer = this.template?.leaderBoard?.presentationQuizPlayer;
    const uniquePlayers = new Map();
    presentationLevelPlayer?.forEach(player => {
      const existingPlayer = uniquePlayers.get(player.playerId);
      if (!existingPlayer || player.score > existingPlayer.score) {
        uniquePlayers.set(player.playerId, player);
      }
    });
    var leaderboardPlayers = Array.from(uniquePlayers.values()).sort((a, b) => b.score - a.score);

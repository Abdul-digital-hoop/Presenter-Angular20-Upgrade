...
export class OpenEndedComponent implements OnInit {
 ...
 isShowResults: boolean = true;
  ...
  //public isShowOptionDetails: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this._workspaceservice.isShowOptionDetails = false;
  }
  ngOnChanges() {
    this.getRgbColr(this.slideTheme?.ThemeBackgroundColor);
    this.profanityFilterCheck(this.profanityWord);
  ...
  // scrollToBottom() {
  //   if (this.slideData?.chart?.name == this.settingVariable?.Flowing) {
  //     const documentHeight = $(document).height();
  //     if (documentHeight >= 800) {
  //       this.incrementalScroll(150, 2000);
  //     }
  //   }
  // }
  ...
  updateChart(value: any) {
    // this.openEndedData = value.filter(item => item.Answer.trim() !== ''&& item.ModerateAnswer == false);
    if(this._workspaceservice.isPreviewMode){
      this.openEndedData = [];
      this.openEndedData = this._workspaceservice.options.filter((item, i, arr) => item.Answer && typeof item.Answer === 'string' && item.Answer.trim() !== '' && item.ModerateAnswer === false && arr.findIndex(opt => opt.AnswerId === item.AnswerId) === i);
    }else{
      this.openEndedData = this._workspaceservice.options.filter(item => item.Answer && typeof item.Answer === 'string' && item.Answer.trim() !== '' && item.ModerateAnswer == false);
    }
    this.openEndedData = this._workspaceservice.profanityWordsChecksForWord(this.openEndedData);
  }

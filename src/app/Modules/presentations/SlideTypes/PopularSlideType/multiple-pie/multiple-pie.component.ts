...
export class MultiplePieComponent implements OnInit {
  ...
  private createDefaultSvg(): void {
    d3.select('div#multiple-pie-chart').select("svg").remove();
    this.svg = d3.select('div#multiple-pie-chart')
      .append('svg')
      .attr('height', '100%')
      .attr('width', '100%')
...
this.svg.transition().duration(500)
  ...
  private animateLabels(data_ready: any, outerArc: any): void {
    ...
    // Bind data to the answer icons inside labelsGroup
    const icons = labelsGroup.selectAll('.answer-icon').data(d => [d]);

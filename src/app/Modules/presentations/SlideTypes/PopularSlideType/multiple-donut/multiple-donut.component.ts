...
export class MultipleDonutComponent implements OnInit, OnChanges {
  ...
  @Input() slideTheme:any;
    //this.data = this._workspaceservice.dynamicChartData(this._workspaceservice.options);
  ...
  transformData(data: any) {
    ...
    if (dataLength === 0 || !this._workspaceservice.slideShowInResults) {
  ...
  updateBar(data:any){
    ...
    else if(this.defaultsvg  || !this._workspaceservice.slideShowInResults){
  ...
  private createDefaultSvg(): void {
    d3.select('div#multiple-donut-chart').select("svg").remove();
    this.svg = d3.select('div#multiple-donut-chart')
      .append('svg')
      .attr('height', '100%')
      .attr('width', '100%')
  ...
  private createSvg(): void {
    d3.select('div#multiple-donut-chart').select("svg").remove();
    if(this._workspaceservice.slideLayoutType=='Default' || this._workspaceservice.slideLayoutType=='Full Image'){
    ...
    const presentationMode = this._workspaceservice.presentationMode;
    ...
    this.svg = d3
      .select("div#multiple-donut-chart")
      .append("svg")
      .attr('height', '100%')
      .attr('width', '100%')
...
private animateLabels(data_ready: any, labelData: any[]): void {
  ...
  private dataReachedChart(data_ready: any) {
    ...
    legend.append('text')
      ...
      .text((d: any) => this._workspaceservice.slideResponseAsPercentage ? `${Math.round(d.data.percentage)}%` : d.data.value);
